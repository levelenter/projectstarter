...
beforeAll(async () => {
  ...
  await con.execute(`DELETE FROM users WHERE mail = ?`, [
    "dai.yamamoto@levelenter.com",
  ]);
});
...
test("DBのデーターベースを一覧する1", async (done) => {
  ...
  const row = db[0].filter((i: RowDataPacket) => {
    return i["Database"] === "app";
  });
  console.log(row[0]);
  expect(row[0].Database).toBe("app");
});
...
test("ユーザーを追加する", async () => {
  ...
  const result1 = await biz.createUserApi("dai", "dai", "dai", "");
  const data1 = result1.data;
  expect(data1.user_name).toBe("dai");

  const queryResult = await con.query<RowDataPacket[]>(
